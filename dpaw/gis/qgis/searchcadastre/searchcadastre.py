...
import configparser, csv, getpass, os, sys, shutil, sqlite3
...
def finalZoom(ids, db_settings):
    ...
    #palyr.setDataDefinedProperties([QgsPalLayerSettings.Size,True,True,'8',''])
    ...
    try:
        QgsProject.instance().removeMapLayer(waBoxLayer.id())
    except:
        pass
    canvas.refresh()
...
class SearchCadastreDialog(QDialog):
    ...
    def validityCheck(self):
        ...
        layerCount = QgsProject.instance().count()
        if layerCount == 0:
            ...
            QgsProject.instance().addMapLayer(waBoxLayer)
